...
abstract class GameRendererMixin implements SynchronousResourceReloader/*, AutoCloseable*/ {
    ...
    private double getActualReachDistance(final double reachDistance) {
        if (this.client.player != null) {
            return AvMMod.getReachDistance(this.client.player, reachDistance);
        }
        return reachDistance;
    }

    @ModifyConstant(method = "updateTargetedEntity(F)V", constant = @Constant(doubleValue = 1.0))
    private double getActualAttackRange0(final double attackRange) {
        if (this.client.player != null) {
            return AvMMod.getAttackRange(this.client.player, attackRange);
        }
        return attackRange;
    }
    ...
    private double getActualAttackRange1(final double attackRange) {
        if (this.client.player != null) {
            return AvMMod.getSquaredAttackRange(this.client.player, attackRange);
        }
        return attackRange;
    }
}
