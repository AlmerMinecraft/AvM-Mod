...
public class DarkPhantomModel<T extends DarkPhantomEntity> extends SinglePartEntityModel<T> {
    ...
    public DarkPhantomModel(ModelPart root) {
        this.root = root;
    ...
    public void setAngles(T phantomEntity, float f, float g, float h, float i, float j) {
        float k = ((float)((DarkPhantomEntity)phantomEntity).getWingFlapTickOffset() + h) * 7.448451f * ((float)Math.PI / 180);
        float l = 16.0f;
